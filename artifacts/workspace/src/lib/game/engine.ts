...
export class ReadyEngine {
  ...
  private drawKara(ctx: CanvasRenderingContext2D) {
    ...
    const bob = this.reduced || teaching ? 0 : Math.sin(t * 2.2) * 3;
    const cx = this.kx;
    ...
    if (obj && !teaching) {
      needle = Math.atan2(obj.y - cy, obj.x - cx);
    } else if (!this.reduced && !teaching) {
      needle = -Math.PI / 2 + Math.sin(t * 1.1) * 0.18;
    }
    const glowA = this.karaMode === "attention" ? 0.5 : 0.35;
    const glow = ctx.createRadialGradient(cx, cy, 2, cx, cy, r + 10);
    glow.addColorStop(0, `rgba(120, 210, 190, ${glowA})`);
    glow.addColorStop(0.55, "rgba(90, 160, 200, 0.12)");
    glow.addColorStop(1, "rgba(90, 160, 200, 0)");
    ctx.fillStyle = glow;
    ctx.beginPath();
    ...
    ctx.strokeStyle = "rgba(138, 112, 64, 0.45)";
    ctx.lineWidth = 1;
    ctx.stroke();
    ...
    ctx.fill();
    if (!this.reduced && !teaching) {
      const ox = cx + Math.cos(t * 1.6) * (r + 5);
      const oy = cy + Math.sin(t * 1.6) * (r + 5);
      ctx.beginPath();
